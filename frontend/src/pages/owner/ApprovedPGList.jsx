...
const ApprovedPGList = () => {
    ...
    return (
        <div>
            <Typography variant="h4" sx={{ mb: 2 }}>Approved PGs</Typography>

            {pgs.map(pg => (
                <Paper key={pg._id} sx={{ p: 2, mb: 2 }}>
                    <Typography><strong>Name:</strong> {pg.name}</Typography>
                    <Typography><strong>Location:</strong> {pg.location}</Typography>
                    <Typography><strong>Price:</strong> ₹{pg.price}</Typography>
                    <Button variant="contained" sx={{ mt: 1 }} onClick={() => handleView(pg)}>
                        View Details
                    </Button>
                </Paper>
            ))}
